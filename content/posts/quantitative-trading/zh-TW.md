...
執行回測我想應該是最簡單的部分，到處都是工具，也有很多的教學文章。反而是我們時常拿到資料就直接使用，忽略了資料本身可能存在的問題，或是回測的方式不恰當造成結果不正確。作者提了幾個常見的偏誤：

1. Survivorship Bias：歷史資料中有些公司已經下市，如果回測到這些股票，結果會與現實有大差距。
2. Look-ahead Bias：例如我們無法在收盤前知道「當日」的最高、最低或是收盤價。
3. Data-snooping Bias：策略對過去的事件、資料過度優化，無法適用於未來。
4. 忽略交易成本

## 有系統的管理資金，控制風險

對於我們沒有金融背景的來說，「風險」這個概念似乎有點難以掌握，不是不知道該如何處理風險，就是低估了風險帶來的危害。作者透過一個範例，來凸顯「風險」對於我們追求長期財富累積所造成的問題，同時也讓我們知道我們直覺上時常低估「風險」的影響。

他問，有一支股票每分鐘都有 50% 的機率上漲 1%，也有 50% 的機率下跌 1%。如果我們買了這支股票，長期來說，我們會賺、會賠還是持平？大多數的人直覺上會認為是持平，但實際上我們會賠錢！

那我們該如何處理風險呢？以最大會長期財富為目標，作者告訴我們如何應用 Kelly Formula 來分配資金、決定槓桿大小，甚至是控制風險，避免意外狀況法生時讓我們輸掉大量資金。
