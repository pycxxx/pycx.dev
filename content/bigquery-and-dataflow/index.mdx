---
title: Podcast 資料分析處理 - BigQuery、Dataflow
...
1. 使用者查詢時，即時查詢 BigQuery。不可行
    1. BigQuery 有查詢限制
    2. 速度不允許。雖然 BigQuery 相對其他 OLAP 作法快很多，但還是要個幾秒鐘。一般使用者不會接受。
...
推薦閱讀這篇文章來了解大致的概念：[探索HyperLogLog算法（含Java实现）](https://dqyuan.top/2018/08/22/hyperloglog.html)。
...
[![](./dataflow.png)*Dataflow job graphs*](https://cloud.google.com/dataflow/docs/guides/job-graph)
...
* [探索HyperLogLog算法（含Java实现）](https://dqyuan.top/2018/08/22/hyperloglog.html)
